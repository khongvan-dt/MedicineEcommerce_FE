    ...
    <div class="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
      ...
      <div class="inline-flex items-center">
        <IconField>
          ...
          <InputText type="text" v-model="filterSalesTable.global.value" placeholder="Search"
            :style="{ borderRadius: '2rem' }" class="w-full" />
        </IconField>
        <Button icon="pi pi-upload" class="mx-4 export-target-button" rounded v-tooltip="'Export'"
          @click="exportCSV"></Button>
      </div>
    </div>
    <DataTable ref="salesTableRef" :value="products" dataKey="id" paginator :rows="5"
