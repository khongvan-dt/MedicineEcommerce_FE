...
<template>
  <div class="grid grid-cols-12 gap-4">
    ...
    <div class="col-span-12 md:col-span-5 xl:col-span-3">
      ...
      <div class="card p-0">
        <div class="card">
          <FileUpload ref="fileUploaderRef" id="files-fileupload" name="demo[]" url="./upload.php" accept="image/*"
            customUpload multiple auto class="upload-button-hidden w-full" invalidFileSizeMessage="Invalid File Size"
            invalidFileTypeMessage="Invalid File Type" :maxFileSize="1000000" @select="onSelectedFiles" :pt="{
              header: { class: '!hidden' },
              root: { class: '!border-0' }
            }">
            <template #content>
                ...
                <div v-for="file in uploadFiles" :key="file.name" class="flex flex-wrap gap-8">
                  <div
                    ...
                    <Button icon="pi pi-times"
                      class="remove-button text-sm absolute justify-center items-center cursor-pointer" rounded
                      :style="{ top: '-10px', right: '-10px', display: 'none' }" @click="onRemoveFile(file)"></Button>
                  </div>
                </div>
              </div>
            </template>
            <template #empty>
              <div v-if="uploadFiles.length < 1" @click="onChooseUploadFiles" class="w-full py-4"
                :style="{ cursor: 'copy' }">
                <div class="h-full flex flex-col justify-center items-center">
                  ...
                  <span class="font-medium text-surface-600 dark:text-surface-200 text-md text-center">Drop or select
                    files</span>
                </div>
              </div>
            </template>
