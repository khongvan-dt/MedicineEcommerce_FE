...
<template>
  <div class="card">
    <DataTable ref="tableRef" :value="customers" paginator :rows="10" showCurrentPageReport
      paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown"
      currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries" :rowsPerPageOptions="[10, 25, 50]"
      :globalFilterFields="['name', 'country.name', 'representative.name']" v-model:filters="filterTable">
      <template #header>
        <div class="flex flex-wrap gap-2 items-center justify-between">
          ...
          <Button type="button" icon="pi pi-user-plus" label="Add New" class="w-full sm:w-auto order-none sm:order-1"
            outlined @click="navigateToCreateListPage" />
        </div>
      </template>
      ...
      <Column field="country.name" header="Country" sortable headerClass="whitespace-nowrap" :style="{ width: '25%' }">
        <template #body="{ data }">
          <div class="flex items-center gap-2">
            <img :alt="data.country.name" src="https://primefaces.org/cdn/primevue/images/flag/flag_placeholder.png"
      ...
      <Column field="representative.name" header="Created By" headerClass="whitespace-nowrap" :style="{ width: '25%' }"
        sortable>
        <template #body="{ data }">
      ...
      <Column field="activity" header="Activity" headerClass="whitespace-nowrap" :style="{ width: '25%' }" sortable>
        <template #body="{ data }">
          <ProgressBar :value="data.activity" :showValue="false" :style="{
            height: '.5rem'
          }" />
        </template>
      </Column>
